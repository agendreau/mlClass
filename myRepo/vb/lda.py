...
from numpy import log, exp, ones,array
import numpy
...
class VariationalBayes:
    ...
    def new_phi(gamma, beta, word, count):
        ...
        beta_in = beta[:,word] #extract beta associated with the word
        
        exp_part = numpy.asarray([exp(digam(gamma[i]) - digam(sum(gamma))) for i in range(0,len(gamma))]) #compute the exponent part
        
        new_phi = numpy.multiply(beta_in,exp_part) #new unnormalized phi
       
       #normalize and take into account word count
        return count*numpy.asarray([(i/sum(new_phi)) for i in new_phi])
    ...
    def m_step(self, topic_counts):
        ...
        #sum rows and divide each entry in the row by the sum.
        updated_beta = numpy.asarray([numpy.asarray(row)/sum(row) for row in topic_counts])
        
        

        # TODO: Finish this function!
        #print updated_beta
        self._beta = updated_beta
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--topics_out", help="Where we write topics",
                           type=str, default="topics1.txt", required=False)
