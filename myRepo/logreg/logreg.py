...
class LogReg:
    ...
    def sg_update(self, train_example, iteration, use_tfidf=False):
        ...
        for feature in range(0,len(self.beta)):
            ...
            self.beta[feature] = (self.beta[feature]+ \
                                    self.step(iteration)*(label-probOfPos)*train_example.x[feature])
            if(train_example.x[feature]==0):
                self.last_update[feature] += 1
            else:
                self.beta[feature] = self.beta[feature] *\
                                    pow((1-(2*self.step(iteration)*self.mu)),(self.last_update[feature]+1))
                self.last_update[feature] = 0
